...
describe('correct()', () => {
  ...
  test('soft hyphen перед переносом строки удаляется, слова склеиваются без пробела', () => {
    expect(correct('слово\u00AD\nпродолжение')).toBe('словопродолжение')
    expect(correct('текст\u00AD\nтекст')).toBe('тексттекст')
  })

  test('soft hyphen перед пустой строкой не склеивается', () => {
    const source = 'заголовок\u00AD\n\nследующий абзац'
    expect(correct(source)).toBe('заголовок\n\nследующий абзац')
  })

  test('soft hyphen в конце последней строки удаляется', () => {
    expect(correct('слово\u00AD\n')).toBe('слово\n')
  })

  test('soft hyphen в середине текста просто удаляется', () => {
    expect(correct('слово\u00ADтекст')).toBe('словотекст')
    expect(correct('текст\u00AD продолжение')).toBe('текст продолжение')
  })

  test('soft hyphen не мешает обычной обработке дефисов', () => {
    expect(correct('школь-\nного')).toBe('школьного')  // обычный дефис работает как раньше
  })
...
describe('renumberFootnotes()', () => {
  ...
  test('ссылка перед двоеточием-знаком препинания нумеруется корректно', () => {
    // «пишет[^96]:» — двоеточие вводит цитату, это не определение сноски
    const source =
      'К. Маркс пишет[^96]:\n\n' +
      '[^96]: Маркс К. Экономическо-философские рукописи 1844 года.\n'
    const expected =
      'К. Маркс пишет[^1]:\n\n' +
