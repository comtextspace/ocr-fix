 ...
 *  1.5. Soft Hyphen (U+00AD) удаляется; при переносе строки слова склеиваются без пробела
 *     («слово\u00AD\nпродолжение» → «словопродолжение»). Остальные \u00AD просто удаляются.
...
function correct(str) {
  ...
  // 1.5. Soft Hyphen (U+00AD): удаляем, при переносе строки склеиваем без пробела
  //     «слово\u00AD\nпродолжение» → «словопродолжение» (аналогично правилу 2)
  //     Остальные \u00AD просто удаляются (невидимый символ)
  str = str.replace(/\u00AD\n(?!($|\n))/g, '')  // перенос с soft hyphen → склейка
  str = str.replace(/\u00AD/g, '')  // остальные soft hyphen → удалить
 ...
 *  - URL в ссылках [текст](url) и изображениях ![alt](url) — url не изменяется,
 *    текстовая часть корректируется
...
function correctComtext(str) {
  ...
  // 2b. URL в ссылках [текст](url) и изображениях ![alt](url).
  //     Защищаем только url-часть: «](url)» → «](placeholder)».
  //     Текстовая часть [текст] остаётся и будет скорректирована.
  body = body.replace(/\]\(([^)\n]*)\)/g, (_, url) => `](${protect(url)})`)
...
function renumberFootnotes(str) {
  ...
  // 2. Перенумеровываем ссылки [^xxx].
  //    Определение — это [^xxx]: строго в начале строки (первый проход уже заменил их).
  //    Ссылка — всё остальное, в том числе [^96] в «пишет[^96]:», где «:» —
  //    знак препинания, а не маркер определения.
  //    Два варианта ссылки:
  //      • [^xxx] не в начале строки (предшествует не-\n символ)
  //      • [^xxx] в начале строки, но НЕ перед «:» (редкий случай ссылки в начале абзаца)
  body = body.replace(
    /(?<=[^\n])\[\^[^\]]+\]|^\[\^[^\]]+\](?!:)/gm,
    () => `[^${++counter}]`
  )
